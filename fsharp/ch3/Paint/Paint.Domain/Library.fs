...
module Rectangle =
    ...
    let defaultDimensions = dimensions 0.0 0.0
...
module Circle =
    ...
    let unwrap (radius: Radius) = match radius with
                                  | Radius x -> x

    let unwrapDiameter (radius: Radius) = radius |> unwrap |> (*) 2.0

    let defaultDimensions = dimensions 0.0
...
module Compound =
    ...
    let defaultDimensions = [Rectangle.defaultDimensions;Rectangle.defaultDimensions]
